...
async function verifyGalleryDeleteFlow() {
  ...
  assert.match(source, /<label class="grid min-w-0 gap-1 text-sm font-semibold text-stone-800">\s*撮影日/)
  assert.match(source, /class="w-full min-w-0 rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-normal"/)
  assert.match(source, /class="block box-border w-full min-w-0 max-w-full appearance-none[^"]*"[\s\S]*type="date"/)
