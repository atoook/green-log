...
同一植物の水やり記録は、MVP の日付基準である Asia/Tokyo の同一日に複数作成しない。Backend は重複登録を 409 として拒否し、Frontend は watering state の `hasWateredToday` を使って当日の記録済みボタンを非活性表示にする。
...
- 同じ植物に対する当日 2 回目以降の水やり記録を作成せず、UI では当日記録済みであることを明示する。
...
sequenceDiagram
    ...
    Service->>WateringRepo: check same-day record
    alt Already recorded today
        Service-->>Router: duplicate error
        Router-->>ApiClient: 409 Conflict
        ApiClient-->>Page: keep recorded-today disabled state
...
同日重複判定は `watered_at` を Asia/Tokyo の日付へ変換したうえで、その日の開始から翌日開始前までの既存 record の有無で判定する。
...
| 2.7 | 同日重複記録を作成しない | `WateringService`, `WateringRepository`, `WateringRouter` | 409 error contract | 水やり記録作成 |
| 2.8 | 当日記録済みボタンの非活性表示 | `usePlantWatering`, `UpcomingCareList`, `WateringActionButton` | `hasWateredToday` | 水やり記録作成 |
...
- 水やり記録作成時は、Asia/Tokyo の同一日に同じ plant の record がすでに存在する場合、record を追加せず重複エラーとして扱う。
...
Backend schema は同じ shape を camelCase で返す。`UpcomingCareItem` は既存 `TodayCareItem` と同じ plant summary/state fields を持つ。水やり状態には `hasWateredToday` を含め、当日すでに記録済みかどうかを UI が判定できるようにする。
...
- `WateringActionButton` は `hasWateredToday` または記録成功直後の状態を受け取り、当日記録済みの場合は非活性にして「今日は記録済み」と分かる copy を表示する。
...
- Invariants:
  ...
  - `has_watered_today` は保存せず、`last_watered_at` と request 時点の日付基準から算出する。
  - 同じ owner と plant の WateringRecord は、Asia/Tokyo の同一日に複数作成しない。
...
- 同じ植物に対する当日 2 回目以降の record 作成は 409。
- `/care/upcoming` 取得失敗時は `UpcomingCareList` に局所 error と retry を表示する。
- 水やり記録失敗時は該当操作を再試行可能な状態に戻す。
- 当日記録済み状態では、Frontend は record request を送らず、ボタンを非活性として表示する。
...
- 同じ plant に対する当日 2 回目の record 作成は 409 になり、新しい WateringRecord を作らない for 2.7。
- detail/upcoming response は `hasWateredToday` を返す for 2.8。
...
- `usePlantWatering` は `hasWateredToday` が true のとき record request を送らない。
- `WateringActionButton` は当日記録済み状態で非活性になり、通常の記録可能状態と異なる label を表示する。
