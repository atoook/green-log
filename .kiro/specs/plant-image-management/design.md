...
| 1.2 | 有効画像を対象植物画像として記録 | `PlantPhotoService`, `S3StorageClient`, `PlantPhotoRepository` | Upload API, Register API | Upload And Register |
...
| 5.3 | 確認後削除 | `PlantPhotoService`, `S3StorageClient`, `PlantPhotoRepository` | DELETE API | Delete Photo |
...
| `S3StorageClient` | Backend Storage | S3 upload/delete と public URL 生成 | 1,5,7 | boto3 P0, settings P0 | Service |
...
| Field | Detail |
|-------|--------|
| Intent | S3 object の upload/delete と public URL 生成を storage details に閉じる |
| Requirements | 1.2, 5.3, 7.6 |

**Responsibilities & Constraints**
- `boto3` client/resource を settings から構築する。
- `upload_fileobj` に `UploadFile.file` を渡し、ACL は指定しない。
- `ContentType` は validated MIME type を `ExtraArgs` で渡す。
- public URL は `https://{bucket}.s3.{region}.amazonaws.com/{object_key}` 形式で生成する。
- 将来の署名付き URL はこの component の URL generation 差し替えで吸収する。
...
**Responsibilities & Constraints**
- `storage_key` を受け取り、現在の S3 public URL を返す。
- `PlantRepository` と `PlantPhotoService` から利用されるが、DB へ URL を保存しない。
- 将来の署名付き URL、CloudFront、R2 移行時の差し替え点になる。
- `backend/app/storage/s3.py` に `S3StorageClient` と同居し、S3 設定の読み取りを一箇所に保つ。
...
- `S3StorageClient` が ACL を指定せず `upload_fileobj` と delete を呼び、public URL を object key から生成する。
