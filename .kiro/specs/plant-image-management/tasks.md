...
- [ ] 1. Foundation: S3 upload と UUID 写真基盤を実装可能にする
- [ ] 1.1 Backend の S3 と multipart upload 実行前提を追加する
...
- [ ] 1.2 写真 ID と代表画像参照を UUID text に移行する
...
- [ ] 1.4 S3 object 操作と表示 URL 生成を分離する
...
- [ ] 2. Backend: owner-scoped な写真 lifecycle を提供する
- [ ] 2.1 owner と plant に閉じた写真永続化操作を追加する
  - 対象植物の写真だけを時系列で取得できる。
...
- [ ] 2.2 (P) 画像 upload の検証と object key 発行を実装する
...
- [ ] 2.3 写真登録、一覧、代表画像、削除の domain flow を実装する
...
- [ ] 2.4 写真操作の protected API surface を追加する
...
- [ ] 2.5 既存植物 read model の代表画像互換を object key ベースへ更新する
...
- [ ] 3. Frontend: 植物詳細に画像ギャラリーを統合する
- [ ] 3.1 写真 API client と multipart request helper を追加する
...
- [ ] 4. Integration validation と regression を完了する
- [ ] 4.1 Backend API と owner separation の統合テストを追加する
...
- [ ] 4.2 Frontend の API、state、UI regression を追加する
...
- [ ] 4.3 Migration、smoke、build の最終確認を更新する
